...
public class TexturePainter : MonoBehaviour {
	...
	float brushSize=5.0f; //The size of our brush
	...
	void DoAction(){	
        ...
        if (HitTestUVPosition(ref uvWorldPosition)){
            if (HitTestMixColor(brushContainer.transform.position + uvWorldPosition, ref mixColor))
            {
                brushColor = Color.Lerp(brushColor, mixColor, 0.5f);
            }
    ...
    //Returns a mix color according to a hit in the polygon2D collider
    bool HitTestMixColor(Vector3 uvWorldPos, ref Color mixColor)
    {
        RaycastHit2D hit;
        Vector2 cursorPos = new Vector2(uvWorldPos.x, uvWorldPos.y);
        hit = Physics2D.Raycast(cursorPos, Vector2.zero);
        if (hit.collider == null || hit.collider.GetComponent<SpriteRenderer>() == null) {
            Debug.Log("null");
            return false;
        }
        mixColor = hit.collider.GetComponent<SpriteRenderer>().color;
        return true;
    }
